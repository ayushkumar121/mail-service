...
Error bufio_read_until(BufIO* bio, const char* terminator) {
    ...
    if (bio->overflow.length > 0) {
        sb_push_sv(&bio->read_buf, sb_to_sv(&bio->overflow));
        bio->overflow.length = 0;
    }

    char buf[512];
    while (true) {
        ssize_t term_pos = sv_find(sb_to_sv(&bio->read_buf), terminator);
        if (term_pos != -1) {
            size_t end = term_pos + strlen(terminator);
            String leftover = SV2(bio->read_buf.data + end, bio->read_buf.length - end);

            if (leftover.length > 0) {
                bio->overflow.length = 0;
                sb_push_sv(&bio->overflow, leftover);
            }
            bio->read_buf.length = end;
            break;
        }
